...
class Calculator(ttk.Frame):
    def __init__(self, parent):
        ...
        self.calc_value = ttk.Entry(self.inputs_frame, font=("Segoe UI", 10), state="readonly")
        ...
        ttk.Button(ctrl_frame, text="Edit Selected", command=self.edit_selected).pack(side="left", padx=2)
    ...
    def add_manual_entry(self):
        try:
            ...
            if self.editing_item_id:
                # Update existing
                self.tree.item(self.editing_item_id, values=(f"${amt:.2f}", qty, f"${sub:.2f}"))
                self.editing_item_id = None
