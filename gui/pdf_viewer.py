...
class PDFViewer(ttk.Frame):
    def __init__(self, parent):
        ...
        self.label = ttk.Label(
            self, text="Center Space (Viewer)", font=("Segoe UI", 12, "bold")
        )
        ...
        self.canvas.create_text(
            300,
            200,
            text="Select a PDF from Archive Space",
            fill="white",
            font=("Segoe UI", 12),
            tags="placeholder",
        )
        ...
        self.prev_btn = ttk.Button(
            self.nav_frame, text="< Prev", command=self.prev_page, state="disabled"
        )
        ...
        self.next_btn = ttk.Button(
            self.nav_frame, text="Next >", command=self.next_page, state="disabled"
        )
        self.next_btn.pack(side="left")

        # Zoom Controls
        ttk.Separator(self.nav_frame, orient="vertical").pack(
            side="left", padx=10, fill="y"
        )

        self.zoom_out_btn = ttk.Button(
            self.nav_frame, text="🔍-", width=4, command=lambda: self.change_zoom(-0.2)
        )
        self.zoom_out_btn.pack(side="left", padx=2)

        self.zoom_label = ttk.Label(
            self.nav_frame, text="100%", width=6, anchor="center"
        )
        ...
        self.zoom_reset_btn = ttk.Button(
            self.nav_frame, text="1:1", width=4, command=self.reset_zoom
        )
